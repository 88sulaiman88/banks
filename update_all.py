...
function fil(){{
  ...
  document.querySelectorAll('.card').forEach(c=>{{
    const storeName = c.querySelector('.sn')?.textContent || '';
    const desc = c.querySelector('.cn')?.textContent || '';
    const bankMatch = aB==='all' || c.dataset.bank===aB;
    const searchMatch = !q || fuzzyMatch(storeName, q) || fuzzyMatch(desc, q);
    const ok = bankMatch && searchMatch;
...
# ────────────────────────────────────────────
def main():
    print(f"\n🚀 بدء تحديث كل البنوك — {datetime.now().strftime('%Y-%m-%d %H:%M')}")
    print("  (اضغط Ctrl+C مرة لتخطي البنك الحالي، مرتين للإيقاف الكامل)\n")

    results = []
    skip_all = False
    for bank in BANKS:
        if skip_all:
            results.append((bank["name"], False))
            continue
        try:
            success = run_bank(bank)
        except KeyboardInterrupt:
            print(f"\n⏹️  تم إيقاف التحديث — جاري بناء index.html بالبيانات المتوفرة...")
            results.append((bank["name"], False))
            skip_all = True
            continue
        results.append((bank["name"], success))

    # ─── اجمع البيانات ───
    print(f"\n{'='*50}")
    print("📦 جاري بناء index.html ...")
    ...
    for bank in BANKS:
        ...
        if data:
            ...
            bank_status[bank["id"]] = {"ok": True, "count": len(data)}
            print(f"  ✅ {bank['name']}: {len(data)} عرض")
        else:
            bank_status[bank["id"]] = {"ok": False, "count": 0}
            print(f"  ❌ {bank['name']}: لا توجد بيانات")
    ...
    print(f"\n  {ok_count}/{len(BANKS)} بنك — {len(all_offers)} عرض")
    print(f"  ✅ تم تحديث index.html — افتحه مباشرة في المتصفح")
